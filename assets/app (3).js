...
(() => {
  ...
  const DEFAULT_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyjnshgys2GuuipSKoN2HxU0NezDGVX0s-ZVmCwKXCAYs69jjgqTg7U-XWT-TDUlWO_/exec";
  ...
  // Writes shared config to the Sheet and VERIFIES it landed.
  //
  // Why not a plain fetch POST? Apps Script POSTs must use mode:"no-cors",
  // whose response is opaque — the browser cannot read whether the write
  // succeeded, so failures (bad token, stale deployment, no access) look
  // identical to successes. That silent-failure mode is what makes edits
  // "save" in one browser and never appear in another.
  //
  // So: prefer a JSONP GET to the saveConfig route, which returns a readable
  // response we can check. If the payload is too big for a URL, fall back to
  // the opaque POST and report that the save could NOT be verified.
  const CONFIG_URL_LIMIT = 7000;   // conservative; keeps us well under URL caps
  async function postConfig(scriptUrl, token, key, value) {
    if (!scriptUrl) throw new Error("No Apps Script URL configured \u2014 open Settings \u2192 Data Connection and add it, otherwise changes only save on this device.");
    const body = typeof value === "string" ? value : JSON.stringify(value);
    const qs = "action=saveConfig&token=" + encodeURIComponent(token) +
               "&key=" + encodeURIComponent(key) +
               "&value=" + encodeURIComponent(body);
    // Small enough to verify? Use the JSONP round-trip.
    if ((scriptUrl.length + qs.length) < CONFIG_URL_LIMIT) {
      const sep = scriptUrl.indexOf("?") >= 0 ? "&" : "?";
      const data = await fetchJSONP(scriptUrl + sep + qs, 25e3);
      if (!data || data.ok !== true) {
        throw new Error((data && data.error) ? String(data.error) : "The Apps Script rejected the save.");
      }
      return { verified: true, bytes: body.length };
    }
    // Too large for one URL: send in numbered chunks, each individually
    // acknowledged, and committed + verified on the final chunk. This keeps
    // saves verifiable at ANY size (rosters with headshots included).
    // Slice the RAW string and encode each piece independently. (Slicing an
    // already-encoded string could split a %XX escape across chunks, and Apps
    // Script decodes each parameter on arrival — which would corrupt the data.)
    const CHUNK = 1200;                        // raw chars; ~4x after encoding
    const total = Math.ceil(body.length / CHUNK);
    const id = "w" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
    const sep = scriptUrl.indexOf("?") >= 0 ? "&" : "?";
    let last = null;
    for (let i = 0; i < total; i++) {
      const piece = encodeURIComponent(body.slice(i * CHUNK, (i + 1) * CHUNK));
      const url = scriptUrl + sep + "action=saveChunk&token=" + encodeURIComponent(token) +
                  "&key=" + encodeURIComponent(key) +
                  "&id=" + id + "&i=" + i + "&n=" + total +
                  "&value=" + piece;
      last = await fetchJSONP(url, 25e3);
      if (!last || last.ok !== true) {
        throw new Error((last && last.error) ? String(last.error) : "Chunk " + (i + 1) + " of " + total + " failed.");
      }
    }
    if (!last || last.committed !== true) throw new Error("The save did not complete.");
    return { verified: last.verified === true, bytes: body.length, chunks: total };
  }
  ...
  function AdminPanel({ rows, period, authed, setAuthed, goals, setGoals, officerGoals, setOfficerGoals, weights, setWeights, competition, setCompetition, photos, savePhotos, photoErr, hiddenOfficers, saveHidden, officerEmails, saveEmails, saveSettings, lenders, saveLenders, buyboxTags, saveBuyboxTags, team, saveTeam, dataConnection, saveState }) {
    ...
    return /* @__PURE__ */ React.createElement("div", null,
      ...
      (saveState && saveState.status) ? /* @__PURE__ */ React.createElement("div", { className: "savebar sb-" + saveState.status },
        /* @__PURE__ */ React.createElement("span", { className: "savebar-ic" },
          saveState.status === "ok" ? "\u2713" : saveState.status === "saving" ? "\u21BB" : saveState.status === "warn" ? "!" : "\u2715"),
        /* @__PURE__ */ React.createElement("span", null, saveState.msg)) : null,
      // ===== DATA CONNECTION =====
      ...
      /* @__PURE__ */ React.createElement("div", { className: "card pad", style: { marginBottom: 16 } },
        ...
        /* @__PURE__ */ React.createElement("p", { className: "muted", style: { fontSize: 13, margin: "0 0 14px", lineHeight: 1.5 } }, "The master team roster \u2014 shown under Directory \u2192 Moxie and used by the leaderboard and Pipeline IQ. New hires are added automatically by onboarding; here you can edit anyone, point each person's headshot at an image file (used everywhere), set their Pipeline IQ email, and toggle whether they appear on the leaderboard. The 5-star review count is pulled from the pipeline by name. Headshot tip: put the image in your site's assets/headshots/ folder and enter the path, e.g. assets/headshots/jane-doe.jpg"),
        (team || []).map((t, i) => {
          ...
          return /* @__PURE__ */ React.createElement("div", { className: "tmemb", key: i },
            ...
            /* @__PURE__ */ React.createElement("div", { className: "tmemb-row" },
              /* @__PURE__ */ React.createElement("span", { className: "tmemb-lbl" }, "Headshot"),
              /* @__PURE__ */ React.createElement("input", {
                className: "k", type: "text", placeholder: "assets/headshots/firstname-lastname.jpg  (or a full https:// URL)",
                value: t.photo || "", onChange: (e) => upd("photo", e.target.value), style: { flex: 1 }
              }),
              t.photo ? /* @__PURE__ */ React.createElement("img", {
                className: "tmemb-thumb", src: t.photo, alt: "",
                onError: (e) => { e.target.style.opacity = ".25"; },
                onLoad: (e) => { e.target.style.opacity = "1"; }
              }) : null,
              t.photo ? /* @__PURE__ */ React.createElement("button", { className: "lndr-tx", type: "button", onClick: () => upd("photo", "") }, "Clear") : null),
  ...
  function App() {
    ...
    // Result of the most recent shared-config save, surfaced in the admin panel
    // so a failed sync is visible instead of silent.
    // status: "" | "saving" | "ok" | "warn" | "err"
    const [saveState, setSaveState] = useState({ status: "", msg: "" });
    ...
    const saveSettings = useCallback(async (next) => {
      ...
      adminDirtyRef.current = true;
      let localOk = true;
      try { await STORE.set("moxie:settings", JSON.stringify(payload)); } catch (e) { localOk = false; }
      // Share across all viewers via the sheet — and report whether it landed.
      setSaveState({ status: "saving", msg: "Saving\u2026" });
      try {
        const r = await postConfig(cfg.scriptUrl, ADMIN_PASSCODE, "settings", payload);
        if (r && r.verified) {
          setSaveState({ status: "ok", msg: "Saved to the team \u2014 visible on every device." });
        } else {
          setSaveState({ status: "warn", msg: "Sent, but the sheet didn't confirm it back. Reload in another browser to check." });
        }
      } catch (e) {
        setSaveState({
          status: "err",
          msg: (localOk ? "Saved on this device only \u2014 NOT shared: " : "Not saved: ") + (e && e.message ? e.message : String(e))
        });
      }
    ...
    const saveTeam = useCallback((next) => {
      // Headshots are stored as URLs/paths, never embedded image data. Older
      // records may still hold a base64 data: URI, which bloats the shared
      // config past the Sheet's cell limits — drop those on save so the roster
      // stays small and reliably syncs.
      const cleaned = (next || []).map((t) => (
        (t && typeof t.photo === "string" && t.photo.slice(0, 5).toLowerCase() === "data:")
          ? Object.assign({}, t, { photo: "" })
          : t
      ));
      setTeam(cleaned);
      ...
      saveSettings({ team: cleaned, officerEmails: derivedEmails });
    }, [saveSettings]);
    ...
    const settingsPanel = showCfg && !cfgAuthed ? /* @__PURE__ */ React.createElement("div", { className: "card pad", style: { marginTop: 18, maxWidth: 420 } }, /* @__PURE__ */ React.createElement("b", null, "\u{1F512} Settings locked"), /* @__PURE__ */ React.createElement("div", { className: "muted", style: { fontSize: 13, margin: "6px 0 10px" } }, "Enter the passcode to open settings and admin."), /* @__PURE__ */ React.createElement("input", { className: "k", type: "password", placeholder: "Passcode", value: cfgPass, autoFocus: true, onChange: (e) => { setCfgPass(e.target.value); setCfgErr(false); }, onKeyDown: (e) => { if (e.key === "Enter") { if (cfgPass === ADMIN_PASSCODE) { setCfgAuthed(true); setAdminAuthed(true); setCfgErr(false); } else setCfgErr(true); } } }), cfgErr && /* @__PURE__ */ React.createElement("div", { className: "badge", style: { marginTop: 10, color: "var(--rose)", borderColor: "rgba(255,107,129,.4)" } }, "Incorrect passcode"), /* @__PURE__ */ React.createElement("button", { className: "iconbtn", style: { marginTop: 14 }, onClick: () => { if (cfgPass === ADMIN_PASSCODE) { setCfgAuthed(true); setAdminAuthed(true); setCfgErr(false); } else setCfgErr(true); } }, "Unlock")) : showCfg && cfgAuthed ? /* @__PURE__ */ React.createElement("div", null,
      ...
      /* @__PURE__ */ React.createElement(AdminPanel, { rows, period, authed: adminAuthed, setAuthed: setAdminAuthed, goals, setGoals, officerGoals, setOfficerGoals, weights, setWeights, competition, setCompetition, photos, savePhotos, photoErr, hiddenOfficers, saveHidden, officerEmails, saveEmails, saveSettings, lenders, saveLenders, buyboxTags, saveBuyboxTags, team, saveTeam, dataConnection: dataConnEl, saveState })) : null;
